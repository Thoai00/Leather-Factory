...
let sectionVisible = false; // To track visibility of the section

// Intersection Observer for detecting section visibility
const observer = new IntersectionObserver(
    (entries) => {
        entries.forEach((entry) => {
            sectionVisible = entry.isIntersecting; // True if the section is visible
        });
    },
...
// Wheel event for the circular slider
window.addEventListener("wheel", function (event) {
    if (!sectionVisible || isScrolling) return; // Only scroll if the section is visible
